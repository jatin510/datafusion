...
impl serde::Serialize for LazyMemoryExecNode {
    #[allow(deprecated)]
    fn serialize<S>(&self, serializer: S) -> std::result::Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        ...
        let mut struct_ser = serializer.serialize_struct("datafusion.LazyMemoryExecNode", len)?;
        if let Some(v) = self.schema.as_ref() {
            struct_ser.serialize_field("schema", v)?;
        }
        if !self.sort_exprs.is_empty() {
            struct_ser.serialize_field("sortExprs", &self.sort_exprs)?;
        }
        if self.partition_count != 0 {
...
impl<'de> serde::Deserialize<'de> for LazyMemoryExecNode {
    #[allow(deprecated)]
    fn deserialize<D>(deserializer: D) -> std::result::Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
                ...
                D: serde::Deserializer<'de>,
            {
                ...
                impl<'de> serde::de::Visitor<'de> for GeneratedVisitor {
                        ...
                        E: serde::de::Error,
                    {
                        match value {
                            "schema" => Ok(GeneratedField::Schema),
                            "sortExprs" | "sort_exprs" => Ok(GeneratedField::SortExprs),
                            "partitionCount" | "partition_count" => Ok(GeneratedField::PartitionCount),
                            _ => Err(serde::de::Error::unknown_field(value, FIELDS)),
                        }
                    }
        ...
        struct GeneratedVisitor;
        impl<'de> serde::de::Visitor<'de> for GeneratedVisitor {
            type Value = LazyMemoryExecNode;

            fn expecting(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                formatter.write_str("struct datafusion.LazyMemoryExecNode")
            }

            fn visit_map<V>(self, mut map_: V) -> std::result::Result<LazyMemoryExecNode, V::Error>
                where
                    V: serde::de::MapAccess<'de>,
            {
                let mut schema__ = None;
                let mut sort_exprs__ = None;
                let mut partition_count__ = None;
                while let Some(k) = map_.next_key()? {
                    match k {
                        GeneratedField::Schema => {
                        ...
                        GeneratedField::SortExprs => {
                            if sort_exprs__.is_some() {
                                return Err(serde::de::Error::duplicate_field("sortExprs"));
                            }
                            sort_exprs__ = Some(map_.next_value()?);
                        }
                        GeneratedField::PartitionCount => {
                            if partition_count__.is_some() {
                                return Err(serde::de::Error::duplicate_field("partitionCount"));
                ...
                Ok(LazyMemoryExecNode {
                    schema: schema__,
                    sort_exprs: sort_exprs__.unwrap_or_default(),
                    partition_count: partition_count__.unwrap_or_default(),
                })
            }
        }
        deserializer.deserialize_struct("datafusion.LazyMemoryExecNode", FIELDS, GeneratedVisitor)
    }
}
        ...
        S: serde::Serializer,
    {
        ...
        if let Some(v) = self.physical_plan_type.as_ref() {
            match v {
                ...
                physical_plan_node::PhysicalPlanType::LazyMemory(v) => {
                    struct_ser.serialize_field("lazyMemory", v)?;
                }
            }
        }
        struct_ser.end()
                ...
                impl<'de> serde::de::Visitor<'de> for GeneratedVisitor {
                        ...
                        E: serde::de::Error,
                    {
                        match value {
                            ...
                            "lazyMemory" | "lazy_memory" => Ok(GeneratedField::LazyMemory),
                            _ => Err(serde::de::Error::unknown_field(value, FIELDS)),
                        }
                    }
                }
...
;
                        ...
                        GeneratedField::LazyMemory => {
                            if physical_plan_type__.is_some() {
                                return Err(serde::de::Error::duplicate_field("lazyMemory"));
                            }
                            physical_plan_type__ = map_.next_value::<::std::option::Option<_>>()?.map(physical_plan_node::PhysicalPlanType::LazyMemory)
;
                        }
                    }
                }
